...
export const projects: Project[] = [
  ...
  },
  {
    ...
    story: [
      ...
      "The most useful thing I did wasn't a chart, it was catching my own mistake. Any \"who's really competing for attention\" analysis has to exclude big tech and pre-installed publishers (Google, Meta, Samsung, and the like), since they aren't competition an indie developer can realistically take share from. My first pass at that filter used plain substring matching, and it quietly mislabeled real companies as big tech: \"Vertex Corporation\" and \"Cognex Corporation\" got flagged because they contain \"x corp,\" and every business with \"Operations\" or \"Operating\" in its name got flagged because it contains \"opera.\" Switching to whole-word regex matching and re-verifying against both datasets fixed it. The bug and the fix are both preserved in the code, since knowing a number can quietly be wrong is as important as computing it.",
      "Once that filter was right, the scale of it was still striking: big tech is only 1.6% of the apps in the fresh 2026 scrape but 64% of its installs (38% in the 2022 archive), which is exactly why every competition metric in the analysis is computed with them excluded, and shown that way rather than silently.",
      "The business model underneath the store shifted hard between the two snapshots. Free apps went from 96.9% to 97.7% of the catalogue, and the median price for the paid apps that remain nearly doubled ($1.99 to $3.50). Pure \"free with nothing\" is now a minority strategy (29.7%): hybrid ads-plus-in-app-purchases (29.2%) and IAP-only (24.1%) between them define how most current apps actually make money.",
      "Star ratings turned out to be a weaker signal than I expected, in two different ways. In the 2022 archive, 87% of all 3.45 million apps never received a single rating, so quoting \"the average rating\" across the whole archive is close to meaningless; among the apps that did get rated, the average is a healthier 4.0. And across the current catalogue, plotting every app's rating count against its score shows close to no correlation, apps with a handful of reviews are just as likely to sit at 4.5 stars as apps with millions, which means popularity and quality are measuring two different things, not one.",
      "Developer concentration barely budges once big tech is out of the picture: a Gini coefficient of 0.99 among all 2022 developers drops only to 0.99 among indie ones, and 0.97 to 0.92 in 2026, still extremely winner-take-most. But 86% of indie developers in the current scrape have shipped exactly one app, so the honest comparison for a new developer is the long tail of other single-app publishers, not the market average. Combining every competition, demand, and quality signal into one opportunity score, Word and Racing come out on top for 2026, and that ranking holds whether or not big tech is counted, a genuine indie opportunity rather than an artifact of the filter.",
    ],
    howItWorks: [
      ...
      "Big tech / pre-installed developers (Google, Meta, Samsung, Microsoft, and similar) are detected with whole-word regex matching and excluded from every competition and opportunity metric, after catching and fixing a real substring-matching false-positive bug (see the write-up above) that mislabeled unrelated companies.",
